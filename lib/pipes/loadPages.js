...
module.exports = function(lastResult, pipe, config, filters, callback){
  ...
  try {
    var pagesFilenames = wrench.readdirSyncRecursive(path.join(config.projectDir, pagesPath)).filter(function(filename) {
  ...
  var pages = pagesFilenames.map(function(filename) {
    try {
      var contents = fs.readFileSync(path.join(config.projectDir, pagesPath, filename), 'utf8').split('---');
    } catch (e) {
    ...
    if (page.status == null || page.status != 'publish'){
      return false;
    }
    return true;
  });
